...
def test_shared_auto_routing_mixed_codecs_forces_single_pass_for_both_tracks(monkeypatch, tmp_path):
    ...
    assert config["_two_phase_render_metadata"]["strategy_family"] == "single_pass"
    assert config["_two_phase_render_metadata"]["source_codecs"] == {
        "host": "h264",
        "guest": "hevc",
    }
...
def test_shared_auto_routing_large_h264_pair_uses_batched_gpu(monkeypatch, tmp_path):
    ...
    assert not captured["smart_copy"]
    assert config["_two_phase_render_metadata"]["strategy_family"] == "batched_gpu"
    assert config["_two_phase_render_metadata"]["shared_segment_count"] == 26
...
def test_shared_auto_routing_small_h264_pair_uses_single_pass(monkeypatch, tmp_path):
    ...
    assert not captured["keyframe_probes"]
    assert config["_two_phase_render_metadata"]["strategy_family"] == "single_pass"
    assert config["_two_phase_render_metadata"]["tracks"]["host"]["fps"] == 29.97
    assert config["_two_phase_render_metadata"]["tracks"]["guest"]["fps"] == 25.0
...
def test_manual_override_smart_copy_still_works(monkeypatch, tmp_path):
    ...
    assert config["_two_phase_render_metadata"]["route_mode"] == "manual"
    assert config["_two_phase_render_metadata"]["requested_strategy"] == "smart_copy"
...
def test_shared_auto_routing_one_output_still_renders(monkeypatch, tmp_path):
    ...
    assert config["_two_phase_render_metadata"]["source_codecs"] == {"host": "h264"}
    assert config["_two_phase_render_metadata"]["tracks"]["host"]["strategy_family"] == "single_pass"
...
def test_shared_auto_routing_mixed_codec_single_pass_validation(monkeypatch, tmp_path):
    ...
    assert config["_two_phase_render_metadata"]["strategy_family"] == "single_pass"
    assert "source_codecs" in config["_two_phase_render_metadata"]
